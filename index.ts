interface OperandValues {
    value1: number;
    value2: number;
}

const parseArguments = (args: Array<string>): OperandValues => {
...
const calculator = (a: number, b: number, op: Operation): number | string => {
    ...
    switch (op) {
        case 'multiply':
            return a * b;
        case 'divide':
            if (b === 0) throw new Error('Cannot divide by zero'); // eq. to if (b === 0) { return 'Cannot divide by zero'; }
            return a / b;
        case 'add':
            return a + b;
        case 'subtract':
